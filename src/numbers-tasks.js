...
function getAverage(value1, value2) {
  let result = ((value1 + value2) / 2).toString();
  if (result === 'Infinity') {
...
function roundToPowerOfTen(num, pow) {
  const midResult = num / 10 ** pow;
  const midResult1 = num % 10 ** pow;
  const midResult2 = midResult1.toString()[0];
  return midResult2 < 5
    ? Math.floor(midResult) * 10 ** pow
    : Math.ceil(midResult) * 10 ** pow;
}
...
function getCountOfOddNumbers(number) {
  const natNumber = Math.abs(number);
  return Math.floor((natNumber + 1) / 2);
}
